...
export async function requireAuth(request: NextRequest) {
  ...
  if (!session?.user?.email) {
    logger.warn('Unauthenticated access attempt', { path: request.nextUrl.pathname });
...
export async function requireAdmin(request: NextRequest) {
  ...
  if (!isAdmin(session.user.email)) {
    logger.warn('Unauthorized admin access attempt', { email: session.user.email, path: request.nextUrl.pathname });
