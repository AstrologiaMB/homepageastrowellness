...
import { CycleFamily, CyclePhase } from '@/lib/services/cycles-service';
...
export function StoryModal({ isOpen, onClose, family }: StoryModalProps) {
    ...
    const now = new Date();
    const currentPhaseIndex = phases.findLastIndex(p => p.data?.date && new Date(p.data.date) <= now);
                ...
                <DialogHeader className="p-6 pb-2">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge variant="secondary">
                            {family.metonic_index === 1 ? 'Ciclo Inicial' : `${family.metonic_index - 1}º Retorno Metónico`}
                        </Badge>
                    </div>
                ...
                <ScrollArea className="flex-1 p-6">
                    <div className="relative border-l-2 border-purple-200 ml-4 space-y-10 pb-10">
                        {phases.map((phase, idx) => {
                            ...
                            const hasJournal = phase.data.journal_entries && phase.data.journal_entries.length > 0;
                                ...
                                <div key={idx} className="relative pl-8">
                                    ...
                                    <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 
                                        ${isCurrent ? 'bg-purple-600 border-purple-600 ring-4 ring-purple-100' :
                                            isPast ? 'bg-purple-400 border-purple-400' : 'bg-white border-slate-300'}`}
                                    />
                                                    ...
                                                    phase.data.journal_entries.map(entry => (
                                                        <div key={entry.id} className="border-l-2 border-purple-400 pl-3">
                                                            <div className="flex justify-between text-xs text-muted-foreground mb-1">
                                                                <span>{format(new Date(entry.date), "d MMM HH:mm", { locale: es })}</span>
                                                            </div>
                                                    ...
                                                    <>
                                                        ...
                                                        {phase.data.journal_entries.length > 2 && (
                                                            <p className="text-xs text-muted-foreground text-center">
                                                                +{phase.data.journal_entries.length - 2} entradas más (Click para expandir)
                                                            </p>
                                                        )}
                                                        {phase.data.journal_entries.length <= 2 && (
