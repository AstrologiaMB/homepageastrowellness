...
export function CartaNatalTabla({ chartData }: CartaNatalTablaProps) {
      ...
      <div className="mt-8">
        <Card className="shadow-md">
          <CardContent className="p-6">
            <p className="text-center text-gray-500">No hay datos de carta natal disponibles para mostrar.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  if (typeof chartData.points !== 'object' || typeof chartData.houses !== 'object') {
    return (
      <div className="mt-8">
        <Card className="shadow-md">
          <CardContent className="p-6">
            <p className="text-center text-red-500">Error: Los datos de la carta natal tienen un formato incorrecto.</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const planetNames = ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn', 'Uranus', 'Neptune', 'Pluto', 'True North Node', 'Lilith', 'Chiron'];
              ...
              <TableBody>
                {planets.map(([planetName, planetData]: [string, any]) => {
                  ...
                  if (planetName === 'Sun' || planetName === 'Moon') {
                    console.log(`${planetName}: ${position}° → ${signName} ${decimalToSexagesimal(position)} Casa ${house}`);
                  }
                  ...
                  return (
                    <TableRow key={planetName}>
                      <TableCell>
                        {symbolName === "NNode" ? <AstroSymbol type="lunar" name={symbolName} /> :
                         symbolName === "Lilith" ? <AstroSymbol type="lunar" name={symbolName} /> :
                         symbolName === "Chiron" ? <AstroSymbol type="asteroid" name={symbolName} /> :
