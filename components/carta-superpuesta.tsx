 ...
 */
export function CartaSuperpuesta({
  tropicalData,
  draconicaData,
  chartId = 'carta-superpuesta',
}: CartaSuperpuestaProps) {
  ...
  useEffect(() => {
    if (
      ...
      tropicalData &&
      draconicaData &&
      tropicalData.planets &&
      tropicalData.cusps &&
      draconicaData.planets &&
      draconicaData.cusps
    ) {
      ...
      try {
        ...
        if (chartRef.current) {
          chartRef.current.innerHTML =
            '<p class="text-center text-red-500 p-4">Error al renderizar la carta superpuesta</p>';
        }
      }
    }
        ...
        <CardContent>
          <div className="flex flex-col items-center">
            <p className="text-center text-gray-500 p-4">
              No hay datos suficientes para mostrar la carta superpuesta. Se requieren tanto los
              datos tropicales como dracónicos.
            </p>
          </div>
        </CardContent>
      </Card>
