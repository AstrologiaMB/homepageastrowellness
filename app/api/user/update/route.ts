  ...
  try {
    ...
    try {
      ...
      if (data.birthDate) {
        ...
        birthDateFormatted = new Date(`${dateStr}T${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}:00`);
        console.log("Fecha formateada:", birthDateFormatted.toISOString());
      }
