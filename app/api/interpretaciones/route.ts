...
async function procesarInterpretacionBackground(
  ...
  try {
    ...
    if (ragRequest.tipo === 'draco') {
      try {
        const cruzadaResponse = await fetch(
          `${getApiUrl('FRONTEND_INTERNAL')}/api/cartas/cruzada`,
          {
            method: 'POST',
            headers: {
    ...
    await prisma.interpretacionCache.update({
      ...
      data: {
        ...
        interpretacionesIndividuales: JSON.stringify(
          interpretacionData.interpretaciones_individuales
        ),
    ...
    await prisma.interpretacionCache
      .update({
        where: { id: cacheId },
        data: {
          status: 'FAILED',
  ...
  try {
    ...
    if (!skipCache) {
      ...
      if (cacheExistente) {
        ...
        if (
          cacheExistente.status === 'COMPLETED' &&
          cacheExistente.interpretacionNarrativa &&
          cacheExistente.interpretacionesIndividuales
        ) {
          const diasDesdeCreacion =
            (Date.now() - cacheExistente.createdAt.getTime()) / (1000 * 60 * 60 * 24);
          if (diasDesdeCreacion < 30) {
            console.log('📋 Devolviendo interpretación desde cache (COMPLETED)');
            const interpretacionesIndividuales = JSON.parse(
              cacheExistente.interpretacionesIndividuales
            );
            const interpretacionesCombinadas = combinarInformacionCasa(
        ...
        if (cacheExistente.status === 'PROCESSING') {
          ...
          if (minutosProcesando < 1) {
            ...
            console.log(
              '⚠️ Interpretación cacheada como PROCESSING pero expiró (Stuck > 1min). Reiniciando.'
            );
          }
        }
        ...
        if (cacheExistente.status === 'FAILED') {
          ...
          if (segundosDesdeFallo < 15) {
            console.log(
              '❌ Interpretación fallida recientemente. Deteniendo reintentos inmediatos.'
            );
            return NextResponse.json(
              {
                status: 'FAILED',
